  ...
  async onboardVsb() {
    ...
    for (let ruleNumber in this.rulesNumbers) {
      ...
      for (let inputNumber in this.rulesNumbers[ruleNumber]) {
        ...
        userInput.push({ parameterId: paramid, minValue: parammin, maxValue: parammax })
      }
      if (nsdid == "") {
        break
      } else {
        if (blueprint["translationRules"] == null) {
          blueprint["translationRules"] = [{ nsdId: nsdid, nsdVersion: nsdversion, nsFlavourId: nsflavourid, nsInstantiationLevelId: instlevelid, input: userInput }]
        } else {
